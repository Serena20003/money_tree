...
CRYPTO_SYMBOLS = combined_financial_data[combined_financial_data["Asset Type"] == "Crypto"]["Ticker"].dropna().unique().tolist()
...
def fetch_google_news(crypto_ticker):
    try:
        url = f"https://news.google.com/rss/search?q={crypto_ticker}+cryptocurrency&hl=en-US&gl=US&ceid=US:en"
        ...
        print(f"⚠️ Google News error for {crypto_ticker}: {e}")
        return []
...
with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
    # ✅ Fetch news articles for all Crypto assets in parallel
    all_articles = list(executor.map(fetch_google_news, CRYPTO_SYMBOLS))
...
df_crypto_sentiment = pd.DataFrame(news_data)

# ✅ Save to CSV
df_crypto_sentiment.to_csv("sentiment_crypto.csv", index=False)

print("\n✅ Sentiment Data saved successfully as 'sentiment_crypto.csv'!")
